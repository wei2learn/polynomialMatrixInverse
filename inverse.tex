...
Our algorithm also uses a sequence of kernel basis computations to
diagonalize the input matrix.  The main difference is that our
extension makes use of {\em shifted} kernel bases along with an
alternate method for measuring size during the reduction.  This
allows us to handle efficiently non-generic inputs of arbitrary
dimension.    The main difficulty to overcome is that the degrees
...
of much larger degree than others. To illustrate the new algorithm
...
For this input our algorithm computes the following decomposition.
$$
...
$$
In general, our algorithm returns a list of matrices
...
of our algorithm in terms of a bound $s\geq 1$ for the average column degree.
For some inputs with skewed column degrees $s$ can be considerably smaller
than $d$.  Our algorithm produces the output matrices
...
As an application of our inversion algorithm we show that the
...
background on the tools required for our algorithm. The inversion
algorithm is included in the next section followed in Section
\ref{sec:complexity} by the theorems giving the new complexity
results for multiplying together the $\mathbf{A}_i$ matrices.
The paper ends with a conclusion and topics for future
research.
...
3.7]{zhou:phd2012}.  The notation $\sum\vec{s}$, for any list
...
$\mathbf{B}\in\mathbb{K}[x]^{m\times m}$ be given. 
...
In particular, we always have $1+ \myxi/n \geq 1$.
...
\qed
\end{exm}
We will make use of the following bound on the sizes of shifted kernel
bases~\cite[Theorem~3.4]{ZLS2012}.  
\begin{lem}
...
\end{lem}
We will also need the following result~\cite[Lemma~3.3]{ZLS2012}
...
Let $\mathbf{F} \in \mathbb{K}^{m \times n}$ and $\vec{s}$ be
...
\\ 0&1\end {array} \right],
$$
respectively, with $\mathbf{N}_r$ determined in Example~\ref{example1}.
...
degrees of the previously computed kernels.  The $\vec{s}$-column degrees
...
$\mathbf{N}_{2,\ell}^{(1)}$ are $(5,4)$.
The final step of the recursion gives the
$(5,4)$-minimal kernel bases $\mathbf{N}_{3,\ell}^{(1)}$ and
$\mathbf{N}_{3,r}^{(1)}$. 
$$
...
\textbf{if }$\columnDimension([\mathbf{N}_{l},\mathbf{N}_r])\neq  n$ 
...
\STATE{$\left\{ \mathcal{A}^{(1)},\mathbf{B}_{1}\right\} :=\inv(\mathbf{R}_{u},\cdeg_{\vec{s}} \mathbf{N}_l)$; \\
...
show that that the sum of column degrees of each of the recursive
...
T(\lceil n/2 \rfloor,\myxi) + \sO{(n^{\omega}(1+\myxi/n))}{1}.$$  
...
\item[(b)] $\sum \cdeg_{\vec{t}} \mathbf{N}_{i+1,l}^{(k)}
\le \sum \vec{s}$ 
...
with the new inversion algorithm presented here, and then taking
...
inversion algorithm in terms of a bound $s \geq 1$ for 
...
It is also possible to efficently handle matrices which have both some rows and some columns of
